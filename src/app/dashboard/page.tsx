  ...
  useEffect(() => {
    async function loadData() {
      ...
      const { data: userJobs } = await supabase
        .from('jobs')
        .select('id, status, illustration_url, created_at, error_message')
        .eq('user_id', user.id)
        ...
        for (const job of userJobs) {
          if (job.illustration_url) {
            const { data } = await supabase.storage
              .from('job-files')
              .createSignedUrl(job.illustration_url, 3600)
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <nav className="bg-white border-b border-slate-200 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-6">
            ...
            <Link href="/pricing" className="text-sm font-medium text-purple-600 hover:text-purple-700 transition-colors">
              Buy more
            </Link>
            <button onClick={handleSignOut} className="text-sm font-medium text-slate-700 hover:text-red-500 transition-colors">
              Sign out
            </button>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 grid lg:grid-cols-3 gap-8">
        ...
        {/* Levi deo - Kreiranje nove figurice */}
          ...
          <div className="bg-white p-8 rounded-3xl shadow-xl border border-slate-100 sticky top-24">
            ...
            {credits > 0 ? (
              ...
              <div className="text-center py-8 flex flex-col items-center gap-4">
                ...
                <Link href="/pricing" className="bg-purple-600 hover:bg-purple-700 text-white font-semibold px-6 py-3 rounded-xl shadow-lg shadow-purple-600/30 transition-all">
                  Buy more credits
                </Link>
              </div>
            )}
              ...
              {jobs.map(job => (
                <Link href={`/jobs/${job.id}`} key={job.id} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md transition-shadow group">
                  <div className="aspect-square bg-slate-100 overflow-hidden flex items-center justify-center">
                    {illustrationUrls[job.id] ? (
                      <img src={illustrationUrls[job.id]} alt="Chibi" className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
                    ) : (
