      ...
      <div className="w-full max-w-md bg-white p-8 md:p-10 rounded-3xl shadow-xl border border-slate-100">
        ...
        {sent ? (
          <div className="text-center bg-green-50 border border-green-200 text-green-700 p-4 rounded-xl">
            ...
            <p className="text-sm mt-1">We sent a magic link to <span className="font-medium">{email}</span>. Click it to log in.</p>
          </div>
        ) : (
          <form onSubmit={signInWithMagicLink} className="space-y-4">
            <input
              ...
              required
              className="w-full px-4 py-3.5 border border-slate-300 rounded-xl focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors text-slate-800"
            />
            <button 
              type="submit" 
              disabled={loading}
              className="w-full bg-purple-600 hover:bg-purple-700 text-white font-semibold py-3.5 rounded-xl transition-colors disabled:opacity-50 shadow-lg shadow-purple-600/30"
            >
